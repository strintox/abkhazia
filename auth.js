...
const Auth = {
    ...
    init: function() {
        ...
        auth.onAuthStateChanged(user => {
            if (user) {
                ...
                // Принудительно устанавливаем isAdmin в true для strintox@gmail.com
                if (user.email && user.email.toLowerCase() === this.adminEmail.toLowerCase()) {
                    this.isAdmin = true;
                    console.log('Авторизован как администратор:', user.email);
                } else {
                    this.isAdmin = false;
                    console.log('Авторизован как обычный пользователь:', user.email);
                }
    ...
    updateUI: function() {
        ...
        if (this.currentUser) {
            ...
            if (this.isAdmin && adminPanel && adminPanel.classList.contains('active')) {
        ...
        auth.signInWithEmailAndPassword(email, password)
            .then(() => {
                if (email.toLowerCase() === this.adminEmail.toLowerCase()) {
                    this.isAdmin = true;
                    console.log('Авторизован как администратор через обычный вход');
                }
    ...
    loginAdmin: function(email, password) {
        ...
        auth.signInWithEmailAndPassword(email, password)
            .then(userCredential => {
                if (userCredential.user.email.toLowerCase() === this.adminEmail.toLowerCase()) {
                    this.isAdmin = true;
                    console.log('Авторизован как администратор через админ-форму');
